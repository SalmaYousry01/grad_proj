...
class DatabaseUtilsdoctor {
  static CollectionReference<DoctorDataBase> getUsersCollection() {
    return FirebaseFirestore.instance.collection(DoctorDataBase.COLLECTION_NAME)
        .withConverter<DoctorDataBase>(fromFirestore: (snapshot, options) =>
  ...
  static Future <DoctorDataBase?> readUserFromFiresore(String id) async {
    DocumentSnapshot<DoctorDataBase> user = await getUsersCollection()
        .doc(id)
        .get();
