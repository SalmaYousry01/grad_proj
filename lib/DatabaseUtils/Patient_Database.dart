...
class DatabaseUtilspatient {
  static CollectionReference<MyPatient> getUsersCollection() {
    return FirebaseFirestore.instance.collection(MyPatient.COLLECTION_NAME)
        .withConverter<MyPatient>(fromFirestore: (snapshot, options) =>
  ...
  static Future <MyPatient?> readUserFromFiresore(String id) async {
    DocumentSnapshot<MyPatient> user = await getUsersCollection()
        .doc(id)
        .get();
